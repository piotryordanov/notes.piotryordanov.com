...
const BrainNote = ({ note }) => {
  ...
  if (note.inboundReferenceNotes != null) {
    ...
    references = note.inboundReferenceNotes.map((reference) => (
      <Link
        ...
        to={reference.slug === 'about' ? `%5Cabout` : `/%5C${reference.slug}`} // hack
        key={reference.slug}
      >
        <div className="py-2">
          <h5 className="text-orange-700">{reference.title}</h5>
  ...
  const AnchorTagWithPopups = (props) => (
    <components.a {...props} popups={popups} noPopups={true} />
  );
    ...
    <MDXProvider components={{ ...components, a: AnchorTagWithPopups }}>
      ...
      <div className="refs-box bg-indigo-100 text-gray-600 rounded-lg mb-4 p-4">
        ...
        <p className="text-sm m-0">
          ...
          <a target="_blank" href="https://twitter.com/messages/compose?recipient_id=1143514457215307776">direct message</a>{' '}
          on Twitter or an <a href="mailto:piotr@piotyordanov.com">email</a> and we can talk.
        </p>
      </div>
    </MDXProvider>
  );
